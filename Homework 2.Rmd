...
c(x0fit,sqrt(varx0)) 
cat("Estimate:", exp(x0fit), "\n",
    "CI:", exp((x0fit+c(qnorm(0.05),-qnorm(0.05))*sqrt(varx0))), "\n")
```

```{r}
beta0 <-  fitprobit$coefficients[1]
beta1 <- fitprobit$coefficients[2]
betacov <- vcov(fitlogit) 
x0fit <- -beta0/beta1
exp(x0fit)
varx0=betacov[1,1]/(beta1^2)+betacov[2,2]*(beta0^2)/(beta1^4)-2*betacov[1,2]*beta0/(beta1^3)
...
```{r}
beta0 <-  fitcloglog$coefficients[1]
beta1 <- fitcloglog$coefficients[2]
betacov <- vcov(fitlogit) 
x0fit <- -beta0/beta1
